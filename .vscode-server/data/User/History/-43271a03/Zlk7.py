...
def resolve_standard_brand(raw_name):
    if not raw_name:
        return None
    alias = BrandAlias.objects.filter(alias__iexact=raw_name.strip()).select_related('brand').first()
...
def resolve_standard_categories(gender, cat1, cat2):
    ...
    c1 = (cat1 or "").strip().upper()
    c2 = (cat2 or "").strip().upper()
    
    std_cat1 = CategoryLevel1Alias.objects.filter(alias__iexact=g).first()
    std_cat2 = CategoryLevel2Alias.objects.filter(alias__iexact=c1).first()
    std_cat3 = CategoryLevel3Alias.objects.filter(alias__iexact=c2).first()
...
def resolve_standard_origin(origin_name):
    if not origin_name or origin_name.strip() == "":
        return None
    alias = CountryAlias.objects.filter(origin_name__iexact=origin_name.strip()).select_related('standard_country').first()
...
def convert_or_update_product(raw_product):
    ...
    origin_for_save = origin_input if origin_input else "-"  # 실제 저장용

    std_origin = resolve_standard_origin(origin_input) if origin_input else "-"  # 치환용
    ...
    product, created = Product.objects.update_or_create(
        ...
        defaults={
            ...
            'origin': std_origin or origin_for_save,  # ✅ 빈 경우 "-" 저장
            'status': 'active',
...
#터미널로 전제 전송 진행
from shop.services.product.conversion_service import bulk_convert_or_update_products
bulk_convert_or_update_products()
