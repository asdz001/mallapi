"""
julian 상품 수집/등록 시스템 (로그 문제 수정 버전)
============================
...
def run_julian_collection(force_full=False):
    ...
    try:
        ...
        logger.info(f"✅ julian 수집 완료: {len(products)}개")
        return len(products)
        
    except Exception as e:
        logger.error(f"❌ julian 수집 실패: {e}")
        import traceback
