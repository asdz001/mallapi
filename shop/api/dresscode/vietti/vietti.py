"""
vietti 상품 수집/등록 시스템 (로그 문제 수정 버전)
============================
🎯 Dresscode API → vietti 상품 자동 수집/등록
📝 실행: python manage.py collect_vietti_products
"""
    ...
    def handle(self, *args, **options):
        ...
        try:
            ...
            if is_full and not options['no_backup']:
                ...
                backup_file = Config.BACKUP_DIR / f"vietti_full_{timestamp}_{len(products)}items.json"
            ...
            logger.info("✅ 수집 완료")
            self.stdout.write(self.style.SUCCESS('✅ vietti 상품 수집 완료'))
...
def run_vietti_collection(force_full=False):
    # ✅ 함수 시작시 로거 생성 (중복 생성 제거)
    logger = get_product_logger(Config.RETAILER_CODE)

    try:
        logger.info("📦 vietti 상품 수집 시작")
        Config.setup()
        ...
        logger.info(f"✅ vietti 수집 완료: {len(products)}개")
        return len(products)
        
    except Exception as e:
        logger.error(f"❌ vietti 수집 실패: {e}")
        import traceback
...
if __name__ == "__main__":
    ...
    # 현재: shop/api/dresscode/vietti/vietti.py
    ...
    try:
        ...
        # 실제 수집 실행
        print("🚀 vietti 수집 실행 시작")
        result = run_vietti_collection()
        ...
        print("   python manage.py collect_vietti_products")
        import traceback
        traceback.print_exc()
        sys.exit(1)