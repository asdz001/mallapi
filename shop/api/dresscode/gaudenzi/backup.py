...
class APIClient:
    ...
    def fetch_products(self, from_time=None, to_time=None, is_full_collection=False):
        if is_full_collection:
            ...
            logger.info(f"📡 부분 수집 API 요청: {params}")
...
class DataProcessor:
    ...
    def process_products(self, products_data, is_full_collection=False):
        logger.info(f"🔄 {'전체' if is_full_collection else '부분'} 상품 처리 시작: {len(products_data)}개")
    ...
    def _classify_products(self, valid_products, existing_products_dict):
        ...
        for product_data in valid_products:
            try:
                ...
                logger.error(f"❌ 상품 분류 실패 {product_data.get('productID')}: {e}")
                self.stats['errors'] += 1
    ...
    def _process_incoming_options(self, incoming_options, existing_options_dict, 
        ...
        for external_option_id, (product_id, option_data, product_data) in incoming_options.items():
            try:
                ...
                logger.error(f"❌ 옵션 처리 실패 {external_option_id}: {e}")
                self.stats['errors'] += 1
    ...
    def _print_stats(self):
        ...
        for key, value in self.stats.items():
            if value > 0:
                logger.info(f"   {key.replace('_', ' ').title()}: {value:,}개")
...
if __name__ == "__main__":
    ...
    logger.info(f"📄 manage.py 존재 확인: {os.path.exists(os.path.join(BASE_DIR, 'manage.py'))}")
    ...
    try:
        ...
        # 실제 수집 실행
        logger.info("🚀 Gaudenzi 수집 실행 시작")
        result = run_gaudenzi_collection()
        logger.info(f"{'✅ 성공' if result > 0 else '❌ 실패'}: {result}개")

    except Exception as e:
        logger.error(f"❌ Django 환경 설정 실패: {e}")
        logger.info(f"📁 계산된 BASE_DIR: {BASE_DIR}")
        logger.info(f"📄 manage.py 존재 확인: {os.path.exists(os.path.join(BASE_DIR, 'manage.py'))}")
        logger.info("📝 Django management command로 실행해보세요:")
        logger.info("   python manage.py collect_gaudenzi_products")
        import traceback
        traceback.print_exc()
        sys.exit(1)