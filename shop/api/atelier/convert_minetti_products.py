...
def safe_float(value):
    try:
        ...
        print(f"❌ [safe_float 변환 오류] value='{value}' → {e}")
        return 0.0
...
def convert_minetti_products(limit=None):
    print(f"📦 [START] MINETTI 수집 시작: {datetime.datetime.now()}")
    atelier = Atelier("MINETTI")
    ...
    with transaction.atomic():
        for idx, good in enumerate(goods_list[:limit] if limit else goods_list):
            ...
            subcat_key = f"{good.get('GenderID')}|{good.get('ParentCategoryID')}|{good.get('CategoryID')}"
            category = subcat_dict.get(subcat_key)
            ...
            if not brand_name:
                ...
                continue
            if not category:
                print(f"⚠️ 카테고리 매핑 실패: {gid} / key={subcat_key}")
                fail += 1
                continue
