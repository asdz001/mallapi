...
# ✅ SKU + item_id 매핑: 최신 JSON에서 바코드 기준으로 불러오기
def load_barcode_to_sku_map_from_latest_json():
...
def send_order(order: Order):
    ...
    sku_map = load_barcode_to_sku_map_from_latest_json()
    ...
    for item in order.items.all():
        ...
        info = sku_map.get(barcode, {})
        sku = info.get("sku", "-")
        item_id = item.product.external_product_id or info.get("item_id") or "000000000000000000000000"
